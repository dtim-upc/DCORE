...
\begin{proof}
  % Let $\delta = \frac{paths_{\ge \tau}(n)}{|\mathcal{P}|}$ and $\sigma = index(p) \cdot \delta$ be constants as in \aref{algo:enumeration}.
\end{proof}

Now, there is only left to prove (3). We start with Lemma~\ref{lemma:paths_equivalence} and Lemma~\ref{lemma:union_of_paths} that will be essential in order to prove Theorem~\ref{theorem:enumeration}.
...
% Redo this part
Let $m = |{paths}_{\ge \tau}(n')| - 1$, $\sigma=0$, and $\delta = |{paths}_{\ge \tau}(n')|$. Then ${paths}_{\ge \tau}(n') = \{\pi_{0}, \pi_{1}, \ldots, \pi_{m}\}$ and by Lemma~\ref{lemma:paths_equivalence} ${paths}_{\ge \tau, \sigma, \delta}(n') = \{\pi_{0}, \pi_{1}, \ldots, \pi_{m}\}$. Now, we proceed to show that when the enumeration is performed on each process $p \in \mathcal{P}$, the union of the outputs is equivalent to ...
